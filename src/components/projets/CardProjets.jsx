...
const CardProjets = (props) => {
  return(
    <Col xs='12' md='6' lg='4' className='mb-5'>
      <Zoom>
        <Card className='cardContainer' {...props.propsCard}>
          <span className='under-maintenance'>{`${props.title}${props.link.includes('winespace') ? ' - en maintenance' : ''}`}</span>
          ...
          {props.link.includes('winespace')
            ? <div className='divImageCard isWinespace'>
          ...
          <CardBody className='bodyCard'>
            ...
            <div className='w-100 float-bottom'>
              {props.link.includes('winespace')
                ? <Button className='isWinespace' disabled>Découvrir <i className="fas fa-angle-right"></i></Button>
                : <a href={props.link} target='_blank' rel='noreferrer noopener'>
                  <Button className='btnCardProjet'>Découvrir <i className="fas fa-angle-right"></i></Button>
                </a>}
            </div>
          </CardBody>
        </Card>
